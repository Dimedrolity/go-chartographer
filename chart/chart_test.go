...
func TestGetFragment_Size(t *testing.T) {
	...
	testSize := func(width, height int) {
		img, err := chart.GetFragment2(tiledEmptyImg, 0, 0, width, height)
		So(err, ShouldBeNil)
	...
	Convey("test minWidth-1", t, func() {
		_, err := chart.GetFragment2(tiledEmptyImg, 0, 0, fragmentMinWidth-1, 1)
		So(errors.As(err, &errSize), ShouldBeTrue)
	})
	Convey("test minHeight-1", t, func() {
		_, err := chart.GetFragment2(tiledEmptyImg, 0, 0, 1, fragmentMinHeight-1)
		So(errors.As(err, &errSize), ShouldBeTrue)
	})
	Convey("test maxWidth+1", t, func() {
		_, err := chart.GetFragment2(tiledEmptyImg, 0, 0, fragmentMaxWidth+1, 1)
		So(errors.As(err, &errSize), ShouldBeTrue)
	})
	Convey("test maxHeight+1", t, func() {
		_, err := chart.GetFragment2(tiledEmptyImg, 0, 0, 1, fragmentMaxHeight+1)
		So(errors.As(err, &errSize), ShouldBeTrue)
	})
}
